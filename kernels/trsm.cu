...
void trsmGraphLaunch(uint32_t n, float *A, float *b, float *x) {
  ...
  CUDA_CHECK(cudaMemset(x_d, 1, n * sizeof(float)));
...
//   CUDA_CHECK(cudaMalloc(&x_d, N * sizeof(float)));
//
//   CUDA_CHECK(cudaMemcpy(A_d, A, N * N * sizeof(float),
//   cudaMemcpyHostToDevice)); CUDA_CHECK(cudaMemcpy(b_d, b, N * sizeof(float),
//   cudaMemcpyHostToDevice)); CUDA_CHECK(cudaMemset(x_d, 0, N *
//   sizeof(float)));
//
...
//   CUDA_CHECK(cudaDeviceSynchronize());
//
//   // CUDA_CHECK(cudaMemcpy(x_gpu, x_d, N * sizeof(float),
...
//
//   CUDA_CHECK(cudaMemcpy(L_d, L, N * N * sizeof(float),
//   cudaMemcpyHostToDevice)); CUDA_CHECK(cudaMemcpy(B_d, B, N * N *
//   sizeof(float), cudaMemcpyHostToDevice)); CUDA_CHECK(cudaMemset(X_d, 0, N *
//   N * sizeof(float)));
//
...
int main() {
  ...
  // Initialize solution vector with RHS 'b' (standard for in-place solvers)
  ...
  // CALL THE BUILDER
  buildTriangularSolverGraph<BLOCK_SIZE>(graph, NUM_BLOCKS, N, d_A, d_x, d_b);
