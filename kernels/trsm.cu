...
////////////////////////////////////////////////////////////////////////////////
// TRSM
// In Cholesky, we want to solve L_ik = A_ik * L_kk^-T, but inverse is expensive.
// Instead solve L_kk * L_ik^T = A_ik, which can be done with TRSM. TRSM uses
// forward substitution in each row. It is sequential in a row, but all rows
// are independent.
...
__global__ void trsm_kernel(uint32_t n, const float* A, float* X, const float* B) {
    trsm(n, A, X, B);
}
...
void test_trsm(uint32_t N) {
    ...
    // Compute B = X_true * L^T (since trsm solves L * X^T = B, so B = X_true * L^T)
    // Alternatively, if trsm is solving row-wise L*x = b, then B = X_true * L^T still fits.
    for (uint32_t i = 0; i < N; ++i) {
        for (uint32_t j = 0; j < N; ++j) {
            float sum = 0.0f;
