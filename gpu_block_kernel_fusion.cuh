...
__device__ void gmem_to_smem(const float *gmem1, const float *gmem2,
    ...
    // Handle vectors
    const float4 *gmem1_4 = reinterpret_cast<const float4*>(gmem1);
    const float4 *gmem2_4 = reinterpret_cast<const float4*>(gmem2);
...
__global__ void block_kernel(const float *A, float *L, // input matrix, Chol matrix
    ...
    for (uint32_t i = j + 1 + blockIdx.x; i < n / m; i += gridDim.x) {
        ...
        float *Aij = smem;
        // trsm_small::block_trsm_reuse<W, m, m>(Ljj, Lij, Aij, n, n); // A, X, B
        trsm_small::block_trsm<m, m>(Ljj, Lij, Aij, n, n);
    }
}
