...
// TL+ {"header_files": ["utils.cuh", "cholesky_naive.cuh", "cholesky_256x256_a.bin"]}
// TL+ {"workspace_files": ["test_256x256x256_a.bin", "test_a_16x3072.bin"]}
#include <chrono>
#include <cstdint>
#include <cstdio>
...
int main(int argc, char **argv) {
    ...
    std::ifstream file("test_256x256x256_a.bin", std::ios::binary);
    std::vector<float> data(256*256);
    file.read(reinterpret_cast<char *>(data.data()), data.size() * sizeof(float));
    if (file.fail()) {
        std::cerr << "Failed to read " << "test_a_16x3072.bin" << std::endl;
        std::abort();
    }
    for (int i = 0 ; i < 5; i ++) {
        for (int j = 0; j < 5; j ++) {
